...
### 问题 7：时区不匹配导致漏论文 ❌（2026-03-31 修复）

**问题描述**:
- arXiv 所有时间都是 UTC 时间，比北京时间晚 8 小时
- 脚本直接使用北京时间查询，导致当天发布的论文不在查询范围内
- 周一任务检测不到论文，生成 `no_new_papers` 标记，但实际论文已经存在

**根本原因**:
- arXiv 论文提交时间是 UTC 时间，例如 UTC 3月27日 17:00 发布的论文，对应北京时间是 3月28日 01:00
- 脚本使用北京时间 `2026-03-27 00:00` 到 `2026-03-27 23:59` 查询，实际对应的 UTC 时间是 `2026-03-26 16:00` 到 `2026-03-27 15:59`，漏掉了 UTC 3月27日下午发布的论文

**解决方案**:
```python
# 在 simple-arxiv-search.py 中添加时区转换
# 北京时间当天的论文对应 UTC 前一天 16:00 到 当天 15:59
...
**经验**: 所有第三方API的时间处理都要考虑时区问题，特别是国际服务，最好统一使用UTC时间处理。

---

### 问题 8：周一检索逻辑与预期不符 ❌（2026-03-31 修复）

**问题描述**:
- 原脚本周一检索 3 天范围（周五、周六、周日），但实际 arXiv 周末不更新，只有周五有新论文
- 导致检索范围过大，浪费API调用和AI解读资源

**解决方案**:
- 修改 `daily-papers-cron.sh` 周一逻辑：只检索上周五当天的论文
- `TO_DATE = FROM_DATE`，避免查询无用日期范围

**经验**: 要充分了解上游服务的更新规律，不要做无用功。
